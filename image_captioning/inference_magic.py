        ...
        for prompt in prompts:
            ...
            with torch.no_grad():
                ...
                #   
                    ...
                    cos_sim = pd.DataFrame(cos_sim[:-1]).apply('    '.join)
                    ...
                    captions.pop()
                    captions_table = pd.Series({"captions":' // '.join(captions)})


                    cols = [output_text_without_prompt_series, captions_table, cos_sim_pred, cos_sim,  wav_col]

                    sim_text = pd.DataFrame(pd.concat(cols, axis=0)).T

                    sim_text.columns = ["pred", "captions", "pred_cos_sim", "GT_cos_sims", "Audio"]

...
def get_length_summary_stats(file_list,
                             dataset,
                             json_=None):
    ...
    if dataset == "Clotho":
        ...
        data = [librosa.get_duration(path=file) for file in file_list if os.path.split(file)[-1] in sound_file_suffixes]
...
if __name__ == '__main__':
    ...
    ac_val = get_length_summary_stats(file_list=glob("../softlinks/AudioCaps_data/*.wav"),
                                    dataset="AudioCaps",
                                    json_="AudioCaps/AudioCaps_val.json")

    ac_test = get_length_summary_stats(file_list=glob("../softlinks/AudioCaps_data/*.wav"),
                                    dataset="AudioCaps",
                                    json_="AudioCaps/AudioCaps_test.json")   

    clotho_test = get_length_summary_stats(file_list=glob("Clotho/test_sounds/*.wav"),
                                       dataset="Clotho")

    sum_stats = pd.concat([ac_val,
               ac_test,
               clotho_test], axis=1).\
                round(1)
    
    sum_stats.columns = ["AudioCaps Validation Set",
                         "AudioCaps Test Set",
                         "Clotho Test Set"]
